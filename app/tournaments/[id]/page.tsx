...
export default function TournamentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [groupAStandings, setGroupAStandings] = useState<RoundRobinStanding[]>([]);
  const [groupBStandings, setGroupBStandings] = useState<RoundRobinStanding[]>([]);
  ...
  const fetchTournamentData = async () => {
    try {
      ...
      if (tournamentData.roundRobinMatches.length > 0) {
        if (tournamentData.type === 8) {
          // For 8-player tournaments, calculate group standings separately
          const groupA = calculateRoundRobinStandings(tournamentData.roundRobinMatches, 'A');
          const groupB = calculateRoundRobinStandings(tournamentData.roundRobinMatches, 'B');
          setGroupAStandings(groupA);
          setGroupBStandings(groupB);
        } else {
  ...
  const renderStandingsTable = (standings: RoundRobinStanding[], showAdvance: boolean = false, advanceCount: number = 4) => (
      ...
      <table className="w-full">
        ...
        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
          {standings.map((standing, index) => (
            <tr key={standing.teamKey} className={showAdvance && index < advanceCount ? 'bg-green-50 dark:bg-green-900/20' : ''}>
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {tournament.type === 8 && (groupAStandings.length > 0 || groupBStandings.length > 0) ? (
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
          ...
          {renderStandingsTable(standings, tournament.type !== 4, tournament.type === 4 ? 2 : 4)}
          {tournament.type === 4 && standings.length >= 2 && (
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-4">
              Top 2 teams advance to final
            </p>
          )}
          {tournament.type === 6 && standings.length >= 4 && (
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-4">
      ...
      {tournament.type === 8 ? (
        <div className="grid md:grid-cols-2 gap-6 mb-6">
          {['A', 'B'].map((group) => {
            const groupMatches = tournament.roundRobinMatches.filter((m: Match) => m.group === group);
